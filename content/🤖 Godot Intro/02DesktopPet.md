...
![Gatse](https://media.giphy.com/media/vFKqnCdLPNOKc/giphy.gif)
...
* `AudioStreamPlayer` spielt Sound ab
* **GDScript**: 
...
## Erste Schritte
Wir öffnen zuerst unter `Project > Project Settings` die Optionen für unser Projekt. Unter `Display > Window` lassen sich die Optionen für `Viewport Width` und `Viewport Height` finden. Wir können auch die Suchfunktion unter `Filter Settings` verwenden, um die Optionen schnell zu finden. Diese bestimmen die Größe unseres Programmfensters, d.i die größe des laufenden Spiels.

Da wir schon hier sind, können wir noch eine weitere Einstellung machen, die uns später helfen wird. Oben im Reiter `Input Map` > `Add New Action`: Name ist beliebig, aber wir verwenden einfach "LeftMouseClick" > `Add`. Wir gehen dann zur neu erstellen Kategorie "LeftMouseClick" und clicken rechts auf das Plus-Symbol. Wir klicke danach auf `Listening for Input` und schon hat Godot unseren Mausclick registiert.
